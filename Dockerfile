...
RUN true\
    ...
    && provisioning/install-sw.sh julia-bindist 1.10.6 /opt/julia-1.10 \
    && (cd /opt/julia-1.10/bin && ln -s julia julia-1.10) \
    && provisioning/install-sw.sh julia-bindist 1.11.1 /opt/julia-1.11 \
...
RUN true \
    && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y \
        fdupes libxdmcp6 \
    && apt-get clean && rm -rf /var/lib/apt/lists/* \
    && provisioning/install-sw.sh miniforge3 24.9.0-0 /opt/conda
...
RUN provisioning/install-sw.sh nodejs-bindist 22.11.0 /opt/nodejs
...
RUN apt-get update && apt-get install -y \
    ...
    && wget \
        ...
        https://sourceforge.net/projects/turbovnc/files/3.1/turbovnc_3.1_amd64.deb \
    && dpkg -i virtualgl_3.1_amd64.deb turbovnc_3.1_amd64.deb \
    && rm virtualgl_3.1_amd64.deb turbovnc_3.1_amd64.deb \
    && apt-get clean && rm -rf /var/lib/apt/lists/*
