...
RUN provisioning/install-sw.sh cmake 3.14.3 /opt/cmake
...
RUN true\
    ...
    && provisioning/install-sw.sh julia-bindist 1.1.0 /opt/julia
...
RUN true \
    ...
    && provisioning/install-sw.sh nvidia-libcuda 430.09 /usr/lib64
...
RUN provisioning/install-sw.sh nodejs-bindist 10.15.0 /opt/nodejs
